    ...
    methods: {
      async editStudent() {
        try {
          ...
          await axios.put(`http://192.168.11.149:8000/api/users/${studentId}`, this.editedStudent, {
            headers: {
      ...
      async fetchStudent() {
        try {
          const studentId = this.$route.params.id;
          const token = localStorage.getItem('accessToken');
          const response = await axios.get(`http://192.168.11.149:8000/api/users/${studentId}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
